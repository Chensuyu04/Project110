...
<?php
if ($_GET['p_search'] == '') {
  $sql = 'SELECT * FROM `orders` WHERE `o_settle`<> 3  and `o_settle`< 5 and YEAR(`o_time`)=YEAR(NOw())';
  if ($_GET["settle"] != "") {
    $sql = 'SELECT * FROM `orders` WHERE `o_settle`="' . $_GET["settle"] . '" and YEAR(`o_time`)=YEAR(NOw())';
  }
  if ($_GET["settle"] == 'all') {
    $sql = 'SELECT * FROM `orders` WHERE `o_settle` <> 3  and`o_settle`< 5 and YEAR(`o_time`)=YEAR(NOw())';
  }
} else {
  $sql = "SELECT * FROM `orders` WHERE `o_settle`<> 3  and`o_settle`< 5 and `o_account` LIKE '%" . $_GET['p_search'] . "%' and YEAR(`o_time`)=YEAR(NOw())";
  if ($_GET["settle"] != "") {
    $sql = 'SELECT * FROM `orders` WHERE `o_settle`="' . $_GET["settle"] . '" and `o_account` LIKE "%' . $_GET['p_search'] . '%" and YEAR(`o_time`)=YEAR(NOw())';
  }
}
  ...
  <main>
    ...
    <div class="container w-auto">
      ...
      <form method='get' class='form-inline'>
        ...
        <input type='button' onclick="javascript:location.href='a_memoryorder.php'" value='歷史訂單'></input>
      ...
      <ul class="nav nav-tabs">
        <li class="nav-item">
          <a class="nav-link" href="a_manorder.php?settle=all">全部訂單</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="a_manorder.php?settle=0">等待確認</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="a_manorder.php?settle=1">等待出貨</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="a_manorder.php?settle=2">商品已出貨</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="a_manorder.php?settle=4">等待取消</a>
        </li>
      </ul>
      <?php
    ...
    _END;
      ...
      while ($row = $result->fetch_assoc()) {
        ...
        if ($row["o_settle"] == "0") {
          echo "<td>等待確認</td>";
          echo '<td><input type="date" name="arrtime" value="' . $row["o_arrtime"] . '"></td>';
          echo '<td><input type="date" name="time" value="' . $row["o_finishtime"] . '"></td>';
          echo '<td><button name="settle_1">確認訂單</button></td>';
        } else if ($row["o_settle"] == "1") {
          echo "<td>等待出貨</td>";
          echo '<td><input type="date" name="arrtime" value="' . $row["o_arrtime"] . '"></td>';
          echo '<td><input type="date" name="time" value="' . $row["o_finishtime"] . '"></td>';
          echo '<td><button name="settle_2">出貨確認</button></td>';
        } else if ($row["o_settle"] == "2") {
          echo "<td>商品已出貨</td>";
          echo '<td><input type="date" name="arrtime" value="' . $row["o_arrtime"] . '"></td>';
          echo '<td><input type="date" name="time" value="' . $row["o_finishtime"] . '"></td>';
          echo '<td><button name="settle_3">確認訂單完成</button></td>';
        } else if ($row["o_settle"] == "4") {
          echo "<td>商品已出貨</td>";
          echo '<td><input type="date" name="arrtime" value="' . $row["o_arrtime"] . '"></td>';
          echo '<td><input type="date" name="time" value="' . $row["o_finishtime"] . '"></td>';
          echo '<td><button name="settle_5">確認取消訂單</button></td>';
        }
      ...
      if (isset($_POST["settle_5"])) {
        $query_num = "SELECT * FROM `orderdetails` where `o_id`='" . $_POST["n"] . "' ";
        $result_num = mysqli_query($db_link, $query_num);
        while ($rows_num = $result_num->fetch_array()) {
          //尋找產品庫存
          $product_num = "SELECT * FROM `products` where `p_id`='" . $rows_num['od_pid'] . "'";
          $result_num = mysqli_query($db_link, $product_num);
          $p_num = $result_num->fetch_assoc();
          //計算庫存並更新
          $new_num = $p_num["p_num"] + $rows_num['od_num'];
          $updete_num = "UPDATE `products` SET `p_num`='" . $new_num . "' WHERE `p_id`='" . $p_num["p_id"] . "'";
          $result_up = mysqli_query($db_link, $updete_num);
        }
        ...
        <ul class="pagination pagination-sm">
          ...
          if ($pages > 1) {
            ...
            if ($page == '1') {
              ...
              echo '<li class="page-item"><a class="page-link" href=?page=1&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '&settle=' . $_GET['settle'] . '>首頁</a></li>';
              echo '<li class="page-item"><a class="page-link" href=?page=' . ($page - 1) . '&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '&settle=' . $_GET['settle'] . '>上一頁</a></li>';
            }
            ...
            for ($i = 1; $i <= $pages; $i++) {
              if ($page - 3 < $i && $i < $page + 3) {
                if ($i == $page) {
                  ...
                  echo '<li class="page-item"><a class="page-link" href=?page=' . $i . '&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '&settle=' . $_GET['settle'] . '>' . $i . '</a></li>';
                }
              }
            }
            ...
            if ($page == $pages) {
              ...
              echo '<li class="page-item"><a class="page-link" href=?page=' . ($page + 1) . '&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '&settle=' . $_GET['settle'] . '>下一頁</a></li>';
              echo '<li class="page-item"><a class="page-link" href=?page=' . $pages . '&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '&settle=' . $_GET['settle'] . '>末頁</a></li>';
            }
          }
