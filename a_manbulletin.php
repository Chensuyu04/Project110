...
<?php
if ($_GET["p_search"] == "") {
  $sql = "SELECT * FROM `bulletin` order by `b_settle`,`b_timedown` ASC ";
} else {
  $sql = "SELECT * FROM `bulletin` WHERE `b_title` LIKE '%" . $_GET['p_search'] . "%'";
}
  ...
  <main>
    ...
    <div class="container w-auto">
      ...
      <form method='get'>
        ...
        <input type='button' onclick="javascript:location.href='a_addbull.php'" value='新增公告'></input><br>
    ...
    _END;
      ...
      while ($row = $result->fetch_assoc()) {
        echo '<form method="POST" action="a_editbull.php"><tr>';
        echo "<td>" . $row["b_title"] . "</td>";
        if ($row["time"] == "0000-00-00") {
          echo "<td>待編輯</td>";
        } else {
          echo "<td>" . $row["b_time"] . "</td>";
        }
        if ($row["b_timedown"] == "0000-00-00") {
          echo "<td>待編輯</td>";
        } else {
          echo "<td>" . $row["b_timedown"] . "</td>";
        }
        if ($row["b_settle"] == "1") {
          $date = date('Y-m-d');
          if ($row["b_time"] <= $date && $row["b_timedown"] >= $date) {
            echo "<td>公告中</td>";
          } else if ($row["b_time"] > $date && $row["b_timedown"] > $date) {
            echo "<td>待公告</td>";
          } else if ($row["b_time"] < $date && $row["b_timedown"] < $date) {
            echo "<td>已下架</td>";
          }
        ...
        if ($row["b_top"] == "1")
          ...
          echo "<td>" . "否" . "</td>";
        echo '<td><button name="uppost" class="btn"><i class="fas fa-pencil-alt"></i></td>';
        echo '<td><button name="del" onclick=\'return confirm("是否刪除")\' class="btn"><i class="fas fa-trash-alt"></i></button></td>';
        echo "<input type='hidden' name='n' value='" . $row["b_id"] . "'>";
        echo "</form>";
      }
      ...
      if ($pages > 1) {
        //總頁數>1才顯示分頁選單
        if ($page == '1') {
          echo '<a class="page-link">首頁 </a>';
          echo '<a class="page-link">上一頁 </a>';
        } else {
          echo '<a href=?page=1&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '>首頁</a>';
          echo '<a href=?page=' . ($page - 1) . '&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '>上一頁</a>';
        }
        ...
        for ($i = 1; $i <= $pages; $i++) {
          if ($page - 3 < $i && $i < $page + 3) {
            if ($i == $page) {
              ...
              echo '<a href=?page=' . $i . '&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '>' . $i . '</a>';
            }
          }
        }
        ...
        if ($page == $pages) {
          echo '<a class="page-link">下一頁 </a>';
          echo '<a class="page-link">末頁</a>';
        } else {
          echo '<a href=?page=' . ($page + 1) . '&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '>下一頁</a>';
          echo '<a href=?page=' . $pages . '&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '>末頁</a>';
        }
      }
