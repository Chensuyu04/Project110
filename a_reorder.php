...
<?php
if ($_GET['p_search'] == '') {
  $sql = 'SELECT * FROM `orders` WHERE `o_settle`>= 5';
  if ($_GET["settle"] != "") {
    $sql = 'SELECT * FROM `orders` where `o_settle`="' . $_GET["settle"] . '"';
  }
  if ($_GET["settle"] == 'all') {
    $sql = 'SELECT * FROM `orders` WHERE `o_settle`>= 5';
  }
} else {
  $sql = "SELECT * FROM `orders` WHERE `o_settle`>= 5 and `o_account` LIKE '%" . $_GET['p_search'] . "%'";
  if ($_GET["settle"] != "") {
    $sql = 'SELECT * FROM `orders` where  `o_settle`="' . $_GET["settle"] . '" and `o_account` LIKE "%' . $_GET['p_search'] . '%"';
  }
}
  ...
  <main>
    ...
    <div class="d-flex flex-column flex-shrink-0 p-3 text-white bg-dark h-auto w-auto" style="width: 280px;">
      ...
      <ul class="nav nav-pills flex-column mb-auto">
        ...
        <li>
          <a href="a_reorder.php" class="nav-link text-white active">
            <svg class="bi me-2" width="16" height="16">
              <use xlink:href="#collection" />
            </svg>
            退貨管理
    ...
    <div class="container w-auto">
      ...
      <ul class="nav nav-tabs">
        <li class="nav-item">
          <a class="nav-link" href="a_reorder.php?settle=all">退貨訂單</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="a_reorder.php?settle=5">已取消</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="a_reorder.php?settle=6">退回商品</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="a_reorder.php?settle=7">查驗商品</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="a_reorder.php?settle=8">已退貨</a>
        </li>
      </ul>
      <?php
    ...
    _END;
      ...
      while ($row = $result->fetch_assoc()) {
        ...
        if ($row["o_settle"] == "5") {
          ...
          echo '<td><input type="date" name="arrtime" value="' . $row["o_arrtime"] . '"></td>';
          echo '<td><input type="date" name="time" value="' . $row["o_finishtime"] . '"></td>';
          echo '<td><button name="settle_7">確認商品退回</button></td>';
        } else if ($row["o_settle"] == "7") {
          echo "<td>查驗商品</td>";
          echo '<td><input type="date" name="arrtime" value="' . $row["o_arrtime"] . '">';
          echo '<td><input type="date" name="time" value="' . $row["o_finishtime"] . '">';
          echo '<td><button name="settle_8">確認退貨</button></td>';
      ...
      if (isset($_POST["settle_7"])) {
        ...
        echo "<script>alert('商品已退回');location.href='a_reorder.php';</script>";
      }
      if (isset($_POST["settle_8"])) {
        ...
        $update = "UPDATE `orders` SET `o_settle`='8',`o_finishtime`='" . $_POST["time"] . "'  
            WHERE `o_id` ='" . $_POST["n"] . "'";
        $result = mysqli_query($db_link, $update);
        echo "<script>alert('確認退貨');location.href='a_reorder.php';</script>";
      }
      ?>
