...
if ($_GET['p_search'] == '') {
  ...
  $sql = "SELECT * FROM `products` WHERE `p_name` LIKE '%" . $_GET['p_search'] . "%'";
}
  ...
  <main>
    ...
    <div class="container w-auto">
      ...
      <form method='get' class='form-inline'>
        ...
        <input type='button' onclick="javascript:location.href='a_addproduct.php'" value='新增產品'></input><br><br>
...
<td>&nbsp;價格&nbsp;</td>
<td>&nbsp;成本&nbsp;</td>
<td>庫存數量<font size = '1'><font color = 'red'>( 紅字表示低於安全存量 )</font></font></td>
<td>下架/上架</td>
<td>安全存量</td>
</tr>
</thead>
_END;
      ...
      while ($row = $result->fetch_assoc()) {
        ...
        echo <<<_END
    <form method = 'POST' action = 'a_editproduct.php'><tr align = 'center'>
    <td>$row[p_name]</td>
    <td >$row[p_class]</td>
    <td>$row[p_price]</td>
    <td>$avg_cost</td>
    _END;
        ...
        if ($row['p_num'] <= $row['p_squantity']) {
          echo '<td><font color="red">' . $row['p_num'] . '</font></td>';
        } else {
        ...
        echo <<<_END
    <td>$row[p_squantity]</td>
    <td><input type = 'button' onclick = "javascript:location.href='a_addproductnum.php?p_id=$row[p_id]'" value = '進貨'></input></td>
    <td>
    <button name = 'uppost' class = 'btn'><i class = 'fas fa-pencil-alt'></i></button>
    </td>
    <td><button name = 'del' onclick = \'return confirm("是否刪除")\' class = 'btn'><i class = 'fas fa-trash-alt'></i></button></td>
    <input type = 'hidden' name = 'n' value = $row[p_id]>
    </form>
    _END;
      }
      echo '</tbody>';
        ...
        <ul class="pagination pagination-sm">
          ...
          if ($pages > 1) {
            ...
            if ($page == '1') {
              ...
              echo '<li class="page-item"><a class="page-link" href=?page=1&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '>首頁</a></li>';
              echo '<li class="page-item"><a class="page-link" href=?page=' . ($page - 1) . '&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '>上一頁</a></li>';
            }
            ...
            for ($i = 1; $i <= $pages; $i++) {
              if ($page - 3 < $i && $i < $page + 3) {
                if ($i == $page) {
                  ...
                  echo '<li class="page-item"><a class="page-link" href=?page=' . $i . '&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '>' . $i . '</a></li>';
                }
              }
            }
            ...
            if ($page == $pages) {
              ...
              echo '<li class="page-item"><a class="page-link" href=?page=' . ($page + 1) . '&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '>下一頁</a></li>';
              echo '<li class="page-item"><a class="page-link" href=?page=' . $pages . '&p_search=' . $_GET['p_search'] . '&selectpage=' . $_GET['selectpage'] . '>末頁</a></li>';
            }
          }
      ...
      <div align="left">
        <form method="post" action="repost.php" target="_blank">
          <input type="month" name="start" id="start"><br>
          <input type="text" name="year" id="start" style="height:30">
          <input type="submit" name="repost" value="查看統計" onclick="get(this.id)">
        </form>
      </div>
  </main>
  <script>
