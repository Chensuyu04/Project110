...
<?php
if ($_GET['p_search'] == '') {
  $sql = 'SELECT * FROM `users` where u_name <>"root"';
} else {
  $sql = "SELECT * FROM `users` WHERE `u_account` LIKE '%" . $_GET['p_search'] . "%'";
}
...
<body>
 ...
 _END;
      ...
      while ($row = $result->fetch_assoc()) {
        $ans_sql = "SELECT * FROM `message` where `account`='" . $row["u_account"] . "' ORDER BY `datetime` DESC LIMIT 1";
        $ans_result = mysqli_query($db_link, $ans_sql);
        $ans_row = mysqli_fetch_assoc($ans_result);
        echo '<form method="POST"><tr>';
        echo "<td>" . $row["u_name"] . "</td>";
        echo "<td>" . $row["u_account"] . "</td>";
        if ($ans_row["answer"] != "") {
          echo "<td>" . $ans_row["answer"] . "則訊息</td>";
        } else {
          echo "<td>0則訊息</td>";
        }

        echo "<td><input type = 'button' onclick = javascript:location.href='a_service.php?u_account=$row[u_account]' value = '回應'></td>";
